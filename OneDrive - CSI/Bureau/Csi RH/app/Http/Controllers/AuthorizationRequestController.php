...
{
    Log::info('Méthode getApprovedAuthorizations appelée');
    try {
        // Récupérer l'ID de l'employé connecté
        $employeeId = Auth::user()->employee->id;
        Log::info('Employé ID: ' . $employeeId);

        // Récupérer les demandes d'autorisation approuvées pour l'employé connecté
        $approvedAuthorizations = AuthorizationRequest::where('status', 'approved')
            ->where('type', 'Télétravail')
            ->where('employee_id', $employeeId)
            ->get();

        // Log le résultat
        Log::info('Nombre d\'autorisations récupérées: ' . $approvedAuthorizations->count());

        // Mapper les autorisations pour la réponse
        $events = $approvedAuthorizations->map(function ($authorization) {
            return [
                'id' => $authorization->id,
                'title' => $authorization->type, // Utiliser le champ 'type' si 'title' n'existe pas
                'start' => $authorization->start_date, // Assurez-vous que 'start_date' est de type DateTime
            // Assurez-vous que 'end_date' est de type DateTime
                'backgroundColor' => '#c5dffe' // Utilisez la méthode de la classe
            ];
        });

        return response()->json($events);
    } catch (\Exception $e) {
        Log::error('Erreur lors de la récupération des autorisations approuvées: ' . $e->getMessage());
        return response()->json(['error' => 'Une erreur est survenue.'], 500);
    }
}

public function generateAuthorizationUrl(Request $request)
    {
        $startDate = $request->query('start');
        return response()->json([
            'url' => route('authorizations.create') . '?start=' . urlencode($startDate),
        ]);
    }
