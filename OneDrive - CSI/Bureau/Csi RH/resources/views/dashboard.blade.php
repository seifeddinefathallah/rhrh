...
@section('content')
    ...
    function loadAuthorizations() {
        $.ajax({
            url: SITEURL + "/authorizations/approved",
            dataType: 'json',
            success: function(data) {
                authorizations = data.map(function(item) {
                    return {
                        id: item.id,
                        title: item.title,
                        start: item.start,
                ...
                $('#full_calendar_events').fullCalendar('renderEvents', authorizations, true);
            },
            error: function(xhr, status, error) {
                console.error('Erreur lors du chargement des autorisations:', xhr.responseText);
            }
        });
    }
    ...
    $('#full_calendar_events').fullCalendar({
        ...
        events: function(start, end, timezone, callback) {
            $.when(
            ...
            ).then(function(leaveData, authData) {
                var leaveEvents = leaveData[0].map(function(item) {
                    return {
                        id: item.id,
                        title: item.title,
                        start: item.start,
                ...
                var authEvents = authData[0].map(function(item) {
                    return {
                        id: item.id,
                        title: item.title,
                        start: item.start,
                ...
                var allEvents = leaveEvents.concat(authEvents);
                callback(allEvents);
        ...
        select: function (start, end, allDay) {
            ...
            // Show modal to select demand type
            Swal.fire({
    title: 'Sélectionner un type de demande',
    input: 'select',
    inputOptions: {
    ...
    inputPlaceholder: 'Choisissez un type de demande',
    ...
    inputValidator: (value) => {
        if (!value) {
            ...
            switch (value) {
                case 'leave_requests':
                    ajaxUrl = SITEURL + '/leave-requests/generate-create-url';
                    break;
                case 'authorizations':
                    ajaxUrl = SITEURL + '/authorizations/generateAuthorizationUrl';
                    break;
                case 'events':
                ajaxUrl = SITEURL + '/events/generateCreateEventUrl';
                    break;
                default:
                    Swal.fire({
                        icon: 'error',
                        title: 'Erreur',
                        text: 'Type de demande non reconnu.',
                        confirmButtonColor: '#03428e'
                    });
                    return;
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.url) {
                        ...
                        console.error('Erreur : URL non définie dans la réponse.');
                    }
                },
                error: function(xhr, status, error) {
                    console.error('Erreur lors de la génération de l\'URL:', xhr.responseText);
                }
            });
        }
    }
        ...
        dayRender: function (date, cell) {
            ...
            var formattedDate = $.fullCalendar.formatDate(date, "Y-MM-DD");
